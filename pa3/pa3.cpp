...
int main(int argc, char* argv[]) {
	...
	int image_number = 2; // default image number
...
void TestStats() {
	...
	*px = RGBAPixel(11, 11, 11, 1.0);
	...
	Stats st(testpng);
	// cout << "sumR values: " << endl;
	// for (int i = 0; i < 3; i++) {
	// 	for (int j = 0; j < 3; j++) {
	// 		cout << "[i, j] = [" << to_string(i) << ", " << to_string(j) << "]" << endl;
	// 		cout << "red sum value: " << to_string(st.sumR[i][j]) << endl;
	// 	}
	// }
...
void TestImgTreeBuildRender() {
	...
	cout << "pixels in image: " << to_string(inputimg.width() * inputimg.height()) << endl;

	cout << "leaves in tree: " << to_string(mytree.CountLeaves()) << endl;
	...
	// SCALING...
	// cout << "Rendering ImgTree to output at 1x scale...";
	// PNG outputimg2 = mytree.Render(10);
	// cout << "done." << endl;

	// cout << "Saving rendered PNG to file...";
	// outputimg2.writeToFile(output_img_path + "-r10.png");
	// cout << "done." << endl;
	// ...
