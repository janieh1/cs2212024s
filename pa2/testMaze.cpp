...
TEST_CASE("HexBoard::distance", "[weight=1][part=hexboard]") {
    ...
    // std::cout << "cells in board:" << std::endl;
    // for (auto kv: board.cells) {
    //     std::cout << "{" << std::to_string(kv.first.first) << ", " << std::to_string(kv.first.second) << "}" << std::endl;
    // }
...
TEST_CASE("HexBoard::generate_maze::correct walls removed", "[weight=1][part=hexboard]") {
    ...
    std::cout << "START: {" << std::to_string(board.start_coord.first) << ", " << std::to_string(board.start_coord.second) << "}" << std::endl;
    std::cout << "END: {" << std::to_string(board.end_coord.first) << ", " << std::to_string(board.end_coord.second) << "}" << std::endl;
...
TEST_CASE("HexBoard::generate_maze::correct longest path length stored", "[weight=1][part=hexboard]") {
    ...
    PNG output = DrawMaze::draw_maze(board, 20, "none");
    if (writeoutputs) {
        output.writeToFile("output_images/maze_r4_test.png");
    }
...
TEST_CASE("HexBoard::solve_maze::correct solution vector", "[weight=1][part=hexboard]") {
    ...
    board.solve_maze();
    vector<pair<int, int>> soln_path = {{0, 0}, {0, -1}, {1, -2}, {1, -3}, {2, -4}, {2, -3}, {3, -4}, {3, -3}, {4, -4}, {4, -3}, {3, -2}, {2, -1}, 
    {1, -1}, {1, 0}, {1, 1}, {1, 2}, {2, 2}, {1, 3}, {0, 3}, {0, 2}, 
    {0, 1}, {-1, 2}, {-1, 3}, {-2, 4}, {-2, 3}, {-2, 2}, {-3, 2}, {-2, 1}, {-1, 0}, {-2, 0}, 
    {-3, 0}, {-4, 1}, {-4, 0}, {-3, -1}, {-2, -1}, {-1, -1}, {-1, -2}, {0, -2}, {0, -3}, {0, -4}, {-1, -3}, {-2, -2}};
...
TEST_CASE("HexBoard::solve_maze::correct solution image", "[weight=1][part=hexboard]") {
    ...
    if (writeoutputs) {
        ...
        //output.writeToFile("maze_sol_r4.png");
    }
    PNG expected;
